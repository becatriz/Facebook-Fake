					...
					if(is_array($extend[$key])) {
						...
						foreach($extend[$key] as $key_in) {
							...
							if (stripos($file_contents, $key_in) !== false) {
                    			...
                    			echo addLog($TEXT['_uni-ext-e1'].'<div class="brz-round brz-sand brz-padding brz-border-bold brz-border-red">'.htmlspecialchars($key_in).'</div>',1);$error = 1;
                			}
						}
					} else {
						...
						if (stripos($file_contents, $extend[$key]) !== false) {
							...
							echo addLog($TEXT['_uni-ext-e1'].'<div class="brz-round brz-sand brz-padding brz-border-bold brz-border-red">'.htmlspecialchars($extend[$key]).'</div>',1);
							$error = 1;
               	 		}	
					}	
				}
