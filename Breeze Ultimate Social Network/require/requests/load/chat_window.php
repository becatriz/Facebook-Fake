...
if((isset($_SESSION['username']) && isset($_SESSION['password'])) || (isset($_COOKIE['username']) && isset($_COOKIE['password']))) {
	...
	if(empty($user['idu'])){
		...
		// Display full body		
		$main_body = display('../../../themes/'.$TEXT['theme'].'/html/main/left_large'.$TEXT['templates_extension']).'<style>body{background:#fff !important;};</style>';
	  
	    $TEXT['content'] = $profile->getChatForm($user,$_POST['v1'],$db);

		// Display right body		
		$right_body = display('../../../themes/'.$TEXT['theme'].'/html/main/right_small_no_lang'.$TEXT['templates_extension']).display('../../../themes/'.$TEXT['theme'].'/html/modals/chat_extras'.$TEXT['templates_extension']);
		
		// Temprary hide express bar
		echo $main_body.$right_body.'<style>.brz-express-uni{display:none!important;}</style>';
	}
