...
if((isset($_SESSION['username']) && isset($_SESSION['password'])) || (isset($_COOKIE['username']) && isset($_COOKIE['password']))) {
	...
	if(empty($user['idu'])){
		...
		if(isset($_POST['f']) && !empty($_POST['f']) && is_numeric($_POST['f']) && $_POST['f'] > 0) {
		    ...
		    // Add header
			$TEXT['content'] = display('../../../themes/'.$TEXT['theme'].'/html/modals/notifications_header'.$TEXT['templates_extension']);
			
			// Notifications
		    $TEXT['posts'] = $profile->getNotiicationsAll($_POST['f'],$user['idu'],$filter).'<style>body{background:#fff !important;}</style>';
			...
			if(isset($_POST['t']) && !empty($_POST['t']) && is_numeric($_POST['t']) && $_POST['t'] == 1) {
				...
				$TEXT['content'] = display('../../../themes/'.$TEXT['theme'].'/html/modals/filter_notifications'.$TEXT['templates_extension']);
