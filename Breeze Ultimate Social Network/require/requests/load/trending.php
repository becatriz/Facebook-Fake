...
if((isset($_SESSION['username']) && isset($_SESSION['password'])) || (isset($_COOKIE['username']) && isset($_COOKIE['password']))) {
	...
	if(empty($user['idu'])){
		...
		if($from == 0) {
			...
			// Set to results
			$trends = display('../../../themes/'.$TEXT['theme'].'/html/main/left_large'.$TEXT['templates_extension']);
			...
			$add_filters = ($filter == "1") ? display('../../../themes/'.$TEXT['theme'].'/html/modals/filter_trending'.$TEXT['templates_extension']) : '';
			
			$TEXT['content'] = $add_filters.$ads.$add_title;
		
			// Bind all ads and filters
			$filters = ($filter == "1") ? display('../../../themes/'.$TEXT['theme'].'/html/main/right_small'.$TEXT['templates_extension']) : $TEXT['content'] ;
