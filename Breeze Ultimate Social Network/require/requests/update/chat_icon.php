...
if((isset($_SESSION['username']) && isset($_SESSION['password'])) || (isset($_COOKIE['username']) && isset($_COOKIE['password']))) {
	...
	if(!empty($user['idu'])) {	
		...
		$image_name = $_FILES['chat-icon-file']['name'];     // File name
		$image_size = $_FILES['chat-icon-file']['size'];     // File size
		$image_temp = $_FILES['chat-icon-file']['tmp_name']; // File temp

        // Check whether file is selected			
        if(isset($_FILES['chat-icon-file']) && is_uploaded_file($_FILES['chat-icon-file']['tmp_name'])){
		...
		if($image_size_info && $image_size <= $page_settings['max_img_size']*1000000){
			...
			if($image_res){
				...
				$image_save_folder 	= $page_settings['folder_chat_icons'] . $new_file_name;	
				...
				if(sizeImage($image_res, $image_save_folder, $image_type, $page_settings['max_chat_icons'], $image_width, $image_height, $page_settings['jpeg_quality'] )){	            
					...
					// Update chat icon
					list($return,$updated_image) = $profile->chatIcon($user['idu'],$_POST['chat-icon-form-id'],$new_file_name);
...
<script language="javascript" type="text/javascript">
	window.top.window.chatIconReturn('<?php echo $db->real_escape_string($return); ?>','<?php echo (isset($updated_image)) ? '1' : '0' ;?>');
</script>
<?php }
if(isset($updated_image) && $updated_image) { ?>
<script language="javascript" type="text/javascript">
	window.top.window.chatIconUpdated('<?php echo $db->real_escape_string($updated_image); ?>');
</script>
<?php } 
mysqli_close($db);
?>