...
if((isset($_SESSION['username']) && isset($_SESSION['password'])) || (isset($_COOKIE['username']) && isset($_COOKIE['password']))) {
	...
	if(!empty($user['idu'])) {	
		...
		$image_name = $_FILES['uPc-f-2']['name'];     // File name
		$image_size = $_FILES['uPc-f-2']['size'];     // File size
		$image_temp = $_FILES['uPc-f-2']['tmp_name']; // File temp
	    	
        // Check whether file is selected			
        if(isset($_FILES['uPc-f-2']) && is_uploaded_file($_FILES['uPc-f-2']['tmp_name'])){
		...
		if($image_size_info && $image_size <= $page_settings['max_img_size']*1000000){
			...
			if($image_res){
				...
				$image_save_folder 	= $page_settings['folder_cover_photos'] . $new_file_name;	
				...
				if(sizeImage($image_res, $image_save_folder, $image_type, $page_settings['max_cover_pics'], $image_width, $image_height, $page_settings['jpeg_quality'] )){	            
					...
					// Update cover
					$return = $profile->updateCover($user['idu'],$new_file_name);
...
<script language="javascript" type="text/javascript">
	window.top.window.resetForm("uPc-2");
</script>

<?php if(isset($updated) && $updated) { ?>
<script language="javascript" type="text/javascript">
	window.top.window.loadImageFull('#profile_view_cover_<?php echo $user['idu'] ; ?>','<?php echo $TEXT['installation'];?>/index.php?thumb=1&src=<?php echo $updated; ?>&fol=b&w=1093&h=381&q=100',<?php echo $user['idu'] ; ?>,1);
</script>
<?php } else { ?>
<script language="javascript" type="text/javascript">
