	...
	if(!empty($user['idu'])) {
		...
		if(!$error && $images_count < $page_settings['MAX_IMAGES'] + 1) {
        	...
        	foreach($_FILES["update-status-form-file"]["tmp_name"] as $key=>$tmp_name){
			    ...
			    if($error || $status_update || $youtube_share) {
					break;
				}
			...
			if(!$error) {
				...
				if(isset($images_uploaded) && !empty($images_uploaded)){
					...
					$p_content = 'https://www.youtube.com/embed/'.$youtube_link[1];		    
...
<script language="javascript" type="text/javascript">
	window.top.window.resetElement('#update-status-form-submit','<?php echo $TEXT['_uni-POST']; ?>');
    ...
    window.top.window.resetElement('#post-file-1','<img class="nav-item-text-inverse-reverse-2 brz-img-add-photo" alt="" src="<?php echo $TEXT['DATA-IMG-12']; ?>"></img>&nbsp;<?php echo $TEXT['_uni-Add_photo']; ?>');		
</script>
<?php } else { ?>
<script language="javascript" type="text/javascript">
	window.top.window.resetElement('#update-status-form-submit','<?php echo $TEXT['_uni-POST']; ?>');
