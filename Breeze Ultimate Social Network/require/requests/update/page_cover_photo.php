...
if((isset($_SESSION['username']) && isset($_SESSION['password'])) || (isset($_COOKIE['username']) && isset($_COOKIE['password']))) {
	...
	// Fetch group user
	$page_role = $profile->getPageRole($user['idu'],$_POST['page-cover-form-id']);
	...
	if(!empty($user['idu']) && $page_role['page_role'] > 3) {	
		...
		if($image_size_info && $image_size <= $page_settings['max_img_size']*1000000){
			...
			if($image_res){
				...
				$image_save_folder 	= $page_settings['folder_page_cover_photos'] . $new_file_name;	
				...
				if(sizeImage($image_res, $image_save_folder, $image_type, $page_settings['max_cover_pics'], $image_width, $image_height, $page_settings['jpeg_quality'] )){	            
					...
					// Update cover
					$return = $profile->updatePageCover($user['idu'],$_POST['page-cover-form-id'],$new_file_name);
...
<script language="javascript" type="text/javascript">
	window.top.window.loadImageFull('#page_view_cover_<?php echo $_POST['page-cover-form-id'] ; ?>','<?php echo $TEXT['installation'];?>/index.php?thumb=1&src=<?php echo $updated; ?>&fol=pb&w=1093&h=420&q=100',0,1);
</script>
<?php } else { ?>
<script language="javascript" type="text/javascript">
